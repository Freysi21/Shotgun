...
namespace Shotgun.Controllers
{
		...
		where TRepository : IRepository<TEntity, IDType>
	{
		...
		public virtual async Task<ActionResult<List<TEntity>>> SearchGet
		(
			[FromQuery] Dictionary<string, string[]> dict,
			[FromQuery] PagingQuery page,
			[FromQuery] Dictionary<string, string[]> dateDict,
			[FromQuery] Dictionary<string, bool> orderByDict
		)
		{
			var results = await repository.Search(page, dict, dateDict, orderByDict);
			var metadata = new
			{
				results.TotalCount,
				results.PageSize,
